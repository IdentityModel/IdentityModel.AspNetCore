...
namespace Clients.Bff.InMemoryTests.TestFramework
{
    public class TestBrowserClient : HttpClient
    {
        ...
        class CookieHandler : DelegatingHandler
        {
            ...
            public CookieHandler(HttpMessageHandler next, ITestOutputHelper testOutputHelper)
                : base(next)
            {
            ...
            protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
            {
                ...
                if (response.Headers.Contains("Set-Cookie"))
                {
                    ...
                    if (request.RequestUri.ToString().Contains("api_user_with_useraccesstokenparameters"))
                    {
                        _testOutputHelper.WriteLine($"Cookie:{cookieHeader}");
                    }
        ...
        public TestBrowserClient(HttpMessageHandler handler, ITestOutputHelper testOutputHelper)
            : this(new CookieHandler(handler, testOutputHelper), testOutputHelper)
        {
        }

        private TestBrowserClient(CookieHandler handler, ITestOutputHelper testOutputHelper)
            : base(handler)
        {
            _handler = handler;
            _testOutputHelper = testOutputHelper;
